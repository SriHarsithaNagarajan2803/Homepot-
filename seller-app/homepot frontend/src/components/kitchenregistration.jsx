...
import { Camera, ChefHat, CheckCircle2, AlertCircle } from 'lucide-react';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/api/register-kitchen', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          kitchenName, ownerName, email, password, fssaiNumber, address, specialties: selectedSpecialties
        })
      });
      const data = await response.json();
